...
# Values copied from the vendor's GA metadata and Gradle's release checksum
# endpoints. Updating either toolchain intentionally requires review here.
APPROVED_JAVA_VERSIONS = {"25.0.4+7": "25"}
APPROVED_GRADLE_CHECKSUMS = {
    "9.4.1": {
        "distribution": "2ab2958f2a1e51120c326cad6f385153bb11ee93b3c216c5fccebfdfbb7ec6cb",
...
def check_workflows(errors: list[str]) -> None:
    ...
    for path in sorted((*WORKFLOWS.glob("*.yml"), *WORKFLOWS.glob("*.yaml"))):
        ...
        for runner in re.findall(
            r"^\s*runs-on:\s*['\"]?([^'\"\s#]+)", contents, re.MULTILINE
        ):
            if runner.endswith("-latest"):
                errors.append(f"{relative}: mutable runner label {runner!r}")

        setup_java_count += len(re.findall(r"uses:\s*actions/setup-java@", contents))
        zulu_distribution_count += len(
            re.findall(r"distribution:\s*['\"]?zulu['\"]?", contents)
        )
        java_version_file_count += len(
            re.findall(r"java-version-file:\s*['\"]?\.java-version['\"]?", contents)
        )
        if re.search(r"^\s*java-version:", contents, re.MULTILINE):
            errors.append(f"{relative}: use .java-version instead of inline Java")

        if "./gradlew" in contents:
            gradle_workflow_count += 1
            if "uses: actions/setup-java@" not in contents:
                errors.append(f"{relative}: Gradle runs without actions/setup-java")
            if not re.search(
                r"java-version-file:\s*['\"]?\.java-version['\"]?", contents
            ):
    ...
    if setup_java_count != zulu_distribution_count:
        errors.append(
            "every actions/setup-java step must select the reviewed Zulu distribution "
            f"({setup_java_count} setup steps, {zulu_distribution_count} Zulu inputs)"
        )
...
def check_java_version(errors: list[str]) -> None:
    ...
    expected_major = APPROVED_JAVA_VERSIONS.get(java_version)
    if expected_major is None:
        errors.append(
            ".java-version must be a reviewed vendor GA LTS build "
    ...
    elif match.group("major") != expected_major:
        errors.append(
...
def check_dependency_versions(errors: list[str]) -> None:
    contents = VERSION_CATALOG.read_text(encoding="utf-8")
    versions_match = re.search(r"(?ms)^\[versions\]\s*(.*?)(?=^\[)", contents)
...
def check_verification_metadata(errors: list[str]) -> None:
    ...
    verify_metadata = root.findtext(
        f"{prefix}configuration/{prefix}verify-metadata", namespaces=namespace
    )
